...
from commands import get_measurement_sf06 ,reset_cable_sf06,syringe_PID
from commands import syringe_parameter_get
# ------------------
# Convert the volume


def unit_to_steps(in_value, unit, syringe_area, microsteps=16):
...
# ----------------
# Convert the time
def unit_to_time(in_value, unit):
...
# ----------------
# Convert the time
def time_to_unit(in_value, unit):
...
def speed_to_basis(in_value, unit):
    ...
    # Convert the time
    out_value = unit_to_time(temp_value, time)

    return out_value

# -----------------------------
# Convert speed to another unit
def basis_to_speed(in_value, unit):
    ...
    # Convert the time
    out_value = time_to_unit(temp_value, time)

    return out_value

# ----------------------------------------------
# Read the message coming from the board, if any
def read_message(board, timeout=1):
...
def getSpeed(in_speed, unit, syringe_area, microsteps=16):
    ...
    # Convert the time
    out_speed = unit_to_time(out_speed, time)
...
def getAcceleration(in_accel, unit, syringe_area, microsteps=16):
    ...
    # Convert the time
    out_accel = unit_to_time(out_accel, time) # Call it twice to match the time unit
